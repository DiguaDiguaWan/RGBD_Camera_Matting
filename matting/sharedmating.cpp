    ...
    for (int i = 0; i < height; ++i)
    {
        for (int j = 0; j < width; ++j)
        {
            ...
            if (tri[i][j] != 0 && tri[i][j] != 255)
            {
                ...
                //	}
                //}
                //在该点领域进行搜寻前后景，该领域为自定义的KI值
                //满足以下条件，色彩距离与空间距离都小于定义的值
...
void SharedMatting::refineSample()
{
    ...
    for (it = uT.begin(); it != uT.end(); ++it)
    {
        ...
        //cv::Set2D(matte, xi, yj, ScalarAll(ftuples[index].alphar * 255));
        delete[] p;
    }
...
//获取Matte
void SharedMatting::matteProcess()
{
    int h = matte.rows;
    int w = matte.cols;
    int s = matte.step1();
